...
{
    Vec<N, T> negative;
    for (unsigned int i = 0; i<N; ++i) negative.v[i] = -rhs.v[i];
    return negative;
}
...
Vec<N, T> operator*=(Vec<N, T>& lhs, const Vec<N, T> &rhs)
{
    for (unsigned int i = 0; i<N; ++i) lhs.v[i] *= rhs.v[i];
    return lhs;
}
...
Vec<N, T> operator*(const Vec<N, T>& lhs, const Vec<N, T> &rhs)
{
    ...
    for (unsigned int i = 0; i<N; ++i) componentwise_product[i] = lhs.v[i] * rhs.v[i];
