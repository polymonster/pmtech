...
namespace
{
    ...
    u32                s_depth_target = 0;
    u32                s_depth_only_shader = 0;
    u32                s_depth_stencil_state = 0;
    u32                s_triangle_vertex_buffer = 0;
    u32                s_depth_texture_sampler = 0;
    ...
    void* user_setup(void* params)
    {
        ...
        s_depth_target = pen::renderer_create_render_target(tcp);

        // load shaders now requiring dependency on pmfx to make loading simpler.
        s_depth_only_shader = pmfx::load_shader("depth_only");
        ...
        s_depth_texture_sampler = pen::renderer_create_sampler(scp);
    ...
    loop_t user_update()
    {
        ...
        if (1)
        {
            ...
            pmfx::set_technique(s_depth_only_shader, 0);
        ...
        // draw quad
        {
            ...
            pen::renderer_set_texture(s_depth_target, s_depth_texture_sampler, 0, pen::TEXTURE_BIND_PS);

            // draw
            pen::renderer_draw_indexed(6, 0, 0, PEN_PT_TRIANGLELIST);

            // unbind render target from the sampler
            pen::renderer_set_texture(0, s_depth_texture_sampler, 0, pen::TEXTURE_BIND_PS);
        }
